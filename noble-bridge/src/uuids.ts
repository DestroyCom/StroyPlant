...
export const UUIDS = {
  live: {
    ...
    // 2026-09-02 correction — the 2026-08-29 fa07/fa09 "swap" conclusion below was itself wrong.
    // docs/PARROT_BLE_REVERSE_ENGINEERING.md (decompiled official app, "Certain" confidence) names
    // fa07 `UUID_LIVE_LED_STATE` — not a sensor. soilMoisturePercent is derived from fa05
    // (`soilMoistureRaw`, `UUID_LIVE_SOIL_PERCENT_VWC`, percent×10) in parrot.ts instead.
    // temperatureC now reads fa0a (`UUID_LIVE_TEMPERATURE_VALUE`), not fa09
    // (`UUID_LIVE_VMC_VALUE`, moisture-adjacent — confirmed live jumping 46.6%→69.6% right after a
    // real watering trigger, docs/ble-captures/17_test_2SEPT.pklg).
